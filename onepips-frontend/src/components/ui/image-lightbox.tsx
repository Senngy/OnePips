...
export default function ImageLightbox({ src, alt = "Image", onClose }: ImageLightboxProps) {
    ...
    return (
        ...
        >
            ...
            {/* Image */}
            <div
                className={`relative transition-all duration-250 ${visible && !exiting ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}
                onClick={e => e.stopPropagation()}
            ...
            {/* Bottom hint */}
            <p className={`absolute bottom-5 left-1/2 -translate-x-1/2 text-[11px] text-white/40 tracking-widest uppercase transition-all duration-250 ${visible && !exiting ? "opacity-100" : "opacity-0"}`}>
                Cliquer en dehors ou <kbd className="font-mono">Échap</kbd> pour fermer · <kbd className="font-mono">+</kbd> / <kbd className="font-mono">-</kbd> pour zoomer
            </p>
        </div>
    );
}
