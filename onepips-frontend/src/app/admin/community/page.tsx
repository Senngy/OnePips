                ...
                <div className="p-8 space-y-8 max-w-7xl mx-auto w-full">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
                        <div className="bg-surface-container-low p-6 rounded-xl border border-white/5 flex flex-col gap-1">
                            <span className="text-outline text-[10px] font-label uppercase tracking-widest">Total Testimonials</span>
                        ...
                        <div className="bg-surface-container-low p-6 rounded-xl border border-white/5 flex flex-col gap-1">
                            ...
                            <div className="flex items-center gap-2">
                                <span className="text-3xl font-headline font-bold text-on-surface">
                                    {statsLoading ? "-" : (getStatValue("average_rating") / 10).toFixed(1)}
                                </span>
                        ...
                        <div className="bg-surface-container-low p-6 rounded-xl border border-white/5 flex flex-col gap-1">
                            <span className="text-outline text-[10px] font-label uppercase tracking-widest">Global Pips Shared</span>
                            <span className="text-3xl font-headline font-bold text-on-surface">
                            ...
                            <div className="bg-surface-container-low rounded-xl overflow-hidden border border-white/5 shadow-xl">
                                <table className="w-full text-left border-collapse">
                                    ...
                                    <tbody className="divide-y divide-white/5">
                                        {testimonialsLoading ? (
                                            <tr><td colSpan={5} className="px-6 py-5 text-center text-outline">Chargement...</td></tr>
                                        ) : testimonials?.length === 0 ? (
                                            <tr><td colSpan={5} className="px-6 py-5 text-center text-outline">Aucun témoignage</td></tr>
                                        ) : (
                                                    ...
                                                    <td className="px-6 py-5">
                                                        <div className="flex text-primary text-xs">
                                                            {Array.from({ length: 5 }).map((_, i) => (
                                                                <span key={i} className="material-symbols-outlined text-xs" style={{ fontVariationSettings: i < t.rating ? "'FILL' 1" : "'FILL' 0" }}>star</span>
                                                            ))}
                                                        </div>
                                                    </td>
                                                    <td className="px-6 py-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                        {resultsLoading ? (
                            <div className="col-span-full text-outline text-center py-8">Chargement des résultats...</div>
                        ) : results?.length === 0 ? (
                            <div className="col-span-full text-outline text-center py-8">Aucun résultat</div>
                        ) : (
                                ...
                                <div key={r.id} className="bg-surface-container rounded-xl overflow-hidden group border border-white/5">
                                    ...
                                    <div className="p-5 space-y-3">
                                        <div className="flex justify-between items-start">
                                            <span className="text-xl font-headline font-bold text-primary">+{r.gain}%</span>
